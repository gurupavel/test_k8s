...
stage('Build') {
      steps {
        sh cmd:'docker build -t ${image_name} .', name: "Image creation"
      }
    }
    stage('Publish') {
      when {
    ...
    stage('Deploy on kubernetes') {
      steps {
       configFileProvider([configFile(fileId: '35b09e42-54db-47e5-9b53-d08712e763e4', targetLocation: '',variable: 'k8s_settings')]) {
            sh cmd:"kubectl --kubeconfig=$k8s_settings apply -f equivvy_deployment_client_equivvy_webapp_react.yml", name: "KUBE apply"
        }
      }
    }
  }
